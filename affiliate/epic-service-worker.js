...
/* eslint-disable */
"use strict";
// *** sw-prefetch configuration
// the variable here is an array of files that are generated at runtime to point to our CDN assets
var precacheConfig = [
  [
    "https://static-assets-prod.epicgames.com/affiliate/static/webpack/./main.epic-diesel-affiliate.f2e7be631720f35bfe42.js",
    null,
  ],
  [
    "https://static-assets-prod.epicgames.com/affiliate/static/webpack/./main.epic-diesel-affiliate.f2e7be631720f35bfe42.css",
    null,
  ],
  [
    "https://static-assets-prod.epicgames.com/affiliate/static/webpack/./polyfill.epic-diesel-affiliate.f2e7be631720f35bfe42.js",
    null,
  ],
];
// the navigateFallback is a path used when offline and a fetch of type navigation can't reach anywhere.
...
var cachePrefix = "sw-precache-v4-epic-service-worker";
var cacheName =
  cachePrefix +
  "-1.0.0.trunk.3749402.b1550.js-" +
  (self.registration ? self.registration.scope : "");
// *** Start of sw-prefetch code. ***
...
self.addEventListener("install", function (event) {
  console.log("[sw-precache-esw] install", event);
  event.waitUntil(
    caches
      .open(cacheName)
      .then(function (cache) {
...
self.addEventListener("activate", function (event) {
  console.log("[sw-precache-esw] activate", event);
          ...
          caches.keys().then(function (keyList) {
            return Promise.all(
              keyList.map(function (key) {
                console.log("[sw-precache-esw] Validating cache key", key);
                var endIndex = key.length - scope.length;
                if (key !== cacheName && key.indexOf(scope) === endIndex) {
                  console.log(
                    "[sw-precache-esw] Clearing old",
                    scope,
                    "cache:",
                    key
                  );
                  ...
                  console.log(
                    "[sw-precache-esw] Cache is valid",
                    scope,
                    "cache:",
                    key
                  );
  ...
  if (event.request.method === "GET") {
    ...
    if (
      ...
      try {
          ...
          fetch(event.request)
            ...
            .catch(function (e) {
              ...
              return caches
                ...
                .then(function (cache) {
                  return cache
                    ...
                    .then(function (response) {
                      if (response) {
                        console.log(
                          "[sw-precache-esw] navigate fetch responded from cache",
                          url
                        );
                        return response;
                      }
                      console.log(
                        "[sw-precache-esw] fetch failed to retrieve the cached item",
                        url
                      );
                      throw Error(
                ...
                .catch(function (e) {
                  console.warn(
                    '[sw-precache-esw] Couldn\'t serve response for "%s" from cache: %O',
                    url,
                    e.message
                  );
                });
        ...
        console.log(
          "[sw-precache-esw] attempted to respond to fetch with network, then cache but hit exception",
          url
        );
      }
    }
    if (shouldRespond) {
      console.log("[sw-precache-esw] fetch shouldRespond", url);
      try {
        event.respondWith(
          caches
            .open(cacheName)
            .then(function (cache) {
              return cache
                ...
                .then(function (response) {
                  if (response) {
                    console.log(
                      "[sw-precache-esw] fetch responded from cache",
                      url
                    );
                    return response;
                  }
                  ...
                  console.log(
                    "[sw-precache-esw] fetch failed to retieve the cached item",
                    url
                  );
                });
            })
        ...
        console.log(
          "[sw-precache-esw] attempted to respond to fetch with cache but hit exception",
          url
        );
      }
    }
...
// *** End of sw-toolbox code. ***
// Runtime cache configuration, using the sw-toolbox library.
toolbox.options.debug = false;
toolbox.options.successResponses = /^0|([123]\d\d)$/;
toolbox.router.get(
  /^https:\/\/cdn[123]?\.unrealengine\.com\/(?!.*(?:\.webm|\.mp4|\.js|\.css)$).*$/,
  toolbox.cacheFirst,
  {}
);
toolbox.router.get(
  /^https:\/\/.*cdn[123]?\.epicgames\.com\/(?!.*(?:\.webm|\.mp4|\.js|\.css)$).*$/,
  toolbox.cacheFirst,
  {}
);
toolbox.router.get(
  /^https:\/\/static-assets-.*epicgames\.(com|net)\/.*\/static\/(?!.*(?:\.webm|\.mp4|\.js|\.css)$).*$/,
  toolbox.cacheFirst,
  {}
);
